...
impl VMManager {
    ...
    pub fn setup(&self) {
        let kpt = self.0.lock();
        let baddr = kpt.as_ref().unwrap().get_baddr().as_u64();
        ...
        unsafe {
            ...
            MAIR_EL1.set((0xFF << 0) | // AttrIdx=0: normal, IWBWA, OWBWA, NTR
                         (0x04 << 8) | // AttrIdx=1: device, nGnRE (must be OSH too)
                         (0x44 <<16)); // AttrIdx=2: non cacheable

            // (ref. D7.2.91: Translation Control Register)
            TCR_EL1.set((0b00 << 37) | // TBI=0, no tagging
                        (ips  << 32) | // IPS
                        (0b11 << 30) | // TG1=64k
                        (0b11 << 28) | // SH1=3 inner
